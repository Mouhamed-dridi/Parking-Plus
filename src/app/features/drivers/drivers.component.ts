...
export class DriversComponent implements OnInit {
  ...
  handleOk() {
    if (this.newDriver.name && this.newDriver.email) {
      this.driverService.save(this.newDriver as Driver);
      this.drivers = this.driverService.getAll();
      this.isAddModalVisible = false;
    }
  }
  ...
  saveEdit() {
    const driver = this.drivers.find(d => d.id === this.editingDriver.id);
    if (driver) {
      this.driverService.save({ ...driver, ...this.editingDriver } as Driver);
      this.drivers = this.driverService.getAll();
    }
